...
TagLib::String ByteVector::Export(TagLib::ByteVector byteVector, TagLib::String mimeType) {
    ...
    if (cfg.BinaryDataMethod() == Configuration::BinaryDataMethodTypes::IGNORE)
...
std::string ByteVector::PathToString(std::string filePath, std::string fileName) {
    ...
    if (cfg.BinaryDataMethod() == Configuration::BinaryDataMethodTypes::PREFIXED_URL)
        retval = NewRelativeUrl(binaryDataUrlPrefix, fileName);
    else if (cfg.BinaryDataMethod() == Configuration::BinaryDataMethodTypes::ABSOLUTE_URL)
